...
metadata[,
  response := fcase(
    anti_pd_1_response == "Complete Response"   , "CR" ,
...
drop_te <- subset(
  rowData(se),
  !is.na(Hash) &
    (hasUnstrandedExonic == TRUE | hasUnstranded3UTR == TRUE | hasUnstranded5UTR == TRUE),
  select = "Hash",
  drop = TRUE
)
...
# Reorder by group for better visualization
...
plotMDS(
  y,
  col = factor(y$samples$stranded_unstranded_rnaseq),
  pch = 19,
  main = "By Batch",
...
lcpm <- cpm(y, log = TRUE)
lcpm.c <- limma::removeBatchEffect(
  lcpm,
  batch = y$samples$stranded_unstranded_rnaseq,
  design = design0
)
```
...
plotMDS(
  lcpm.c,
  col = factor(y$samples$vital_status),
  pch = 19,
  main = "By Vital Status",
  gene.selection = "common"
)
plotMDS(
  lcpm.c,
  col = factor(y$samples$disease_status),
  pch = 19,
  main = "By Disease Status",
  gene.selection = "common"
)
plotMDS(
  lcpm.c,
  col = factor(y$samples$response),
  pch = 19,
  main = "By Response",
  gene.selection = "common"
)
...
```{r}
png(here(figures, "rle-boxplot.png"), width = 9, height = 6, units = "in", res = 300)
coriell::plot_boxplot2(lcpm.c, metadata = y$samples, fill_by = "response", rle = TRUE, las = 2)
dev.off()

png(here(figures, "lib-density.png"), width = 9, height = 6, units = "in", res = 300)
coriell::plot_density2(lcpm.c, metadata = y$samples, col_by = "response")
dev.off()
```
...
# Transcripts
v1 <- plot_volcano(
  subset(results[[1]], feature_id %like% "^ENST"),
  ...
  labs(title = "PR vs PD")
v2 <- plot_volcano(
  subset(results[[2]], feature_id %like% "^ENST"),
  x_axis_limits = c(-10, 10),
  y_axis_limits = c(0, 4),
  fdr = 0.1
) +
